...
/**
 * Resolves the home-page hero image.
 *
 * If `heroPath` (a gallery-relative path from site.config `heroImage`) matches an image,
 * that image is used; otherwise it falls back to the first image of the `featured`
 * collection. Sharing this resolver keeps the Hero and FeaturedGallery in agreement about
 * which image is the hero (so the grid can exclude it).
 *
 * @param {string} [heroPath] - Gallery-relative path of the desired hero image.
