    ...
    describe('Get Images', () => {
        ...
        describe('Failures', () => {
            it('should fail on a missing gallery file', async () => {
                await expect(getImages({ galleryPath: GALLERY.MISSING })).rejects.toThrow(
                    ImageStoreError,
                );
            });

            it('should fail on invalid gallery file', async () => {
                await expect(getImages({ galleryPath: GALLERY.INVALID })).rejects.toThrow(
                    ImageStoreError,
                );
            });
            ...
            it('should fail on invalid collection', async () => {
                await expect(
                    getImages({ galleryPath: GALLERY.INVALID_COLLECTION }),
